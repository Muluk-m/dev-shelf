...
const API_ENDPOINT = "http://localhost:9088/website-check/analyze";
		...
		<div className="container mx-auto py-8 px-4">
			...
			{result && (
				<>
							...
							<CardContent className="space-y-4">
								<div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
									...
									{result.meta.description && (
										<div className="md:col-span-2">
											...
											<span className="font-medium">{result.meta.description}</span>
										</div>
									)}
									{result.meta.keywords && (
										<div className="md:col-span-2">
											<span className="text-muted-foreground">Keywords:</span>{" "}
											<span className="font-medium">{result.meta.keywords}</span>
										</div>
									)}
									{result.meta.author && (
										<div>
											<span className="text-muted-foreground">Author:</span>{" "}
											<span className="font-medium">{result.meta.author}</span>
										</div>
									)}
									{result.meta.robots && (
										<div>
											<span className="text-muted-foreground">Robots:</span>{" "}
											<span className="font-medium">{result.meta.robots}</span>
										</div>
									)}
									...
									{result.meta.language && (
										<div>
											...
											<span className="font-medium">{result.meta.language}</span>
										</div>
									)}
										...
										<div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
											...
											{result.meta.og.type && (
												<div>
													...
													<span className="font-medium">{result.meta.og.type}</span>
												</div>
											)}
								...
								{result.meta.twitter &&
									Object.keys(result.meta.twitter).length > 0 && (
										<div className="pt-3 border-t space-y-2">
											<div className="font-semibold text-sm">Twitter Card</div>
											<div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
					...
					{/* Console Logs */}
					{result.console &&
						(result.console.errorCount > 0 || result.console.warningCount > 0) && (
							<Card className="mb-8">
								<CardHeader className="pb-3">
									<CardTitle className="text-lg flex items-center gap-2">
