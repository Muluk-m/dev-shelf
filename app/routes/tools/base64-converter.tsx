...
const encodeBase64 = (value: string, wrapLines: boolean, lineLength: number) => {
	...
	const handleConvert = () => {
		try {
			...
			if (mode === "encode") {
				...
				setOutput(encodeBase64(input, wrapLines, Number.isNaN(parsedLength) ? 76 : parsedLength));
			} else {
		...
		<div className="space-y-8">
				...
				<CardContent className="space-y-6">
					...
					>
						...
						<div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
							...
							<div className="space-y-3">
								<Label htmlFor="base64-output">转换结果</Label>
								<Textarea
									id="base64-output"
									value={output}
									readOnly
			...
			<Card>
				...
				<CardContent className="space-y-2 text-sm text-muted-foreground">
					...
					<p>• 处理 JWT、URL Safe Base64 时，可在结果中手动替换 `+` `/` `=`。</p>
				</CardContent>
			</Card>
		</div>
	);
