    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl p-0 gap-0">
        <DialogHeader className="px-4 py-3 border-b">
          ...
          <DialogDescription className="sr-only">快速搜索和执行命令</DialogDescription>
          ...
          <div className="p-2">
            {Object.keys(groupedCommands).length === 0 ? (
              <div className="flex flex-col items-center justify-center py-8 text-center">
                ...
                <p className="text-sm text-muted-foreground">未找到匹配的命令</p>
              </div>
            ) : (
              <div className="space-y-4">
                {Object.entries(groupedCommands).map(([category, commands], categoryIndex) => {
                  // 计算当前分类之前的命令总数
                  const previousCommandsCount = Object.entries(groupedCommands)
                    ...
                    <div key={category}>
                      ...
                      <div className="space-y-1">
                        {commands.map((command, commandIndex) => {
                          const globalIndex = previousCommandsCount + commandIndex
                          const isSelected = globalIndex === selectedIndex
                          const IconComponent = iconMap[command.icon as keyof typeof iconMap] || Search

                          return (
                            <button
                              key={command.id}
                              onClick={() => handleCommandClick(command, globalIndex)}
                              onMouseEnter={() => handleCommandHover(globalIndex)}
                              className={cn(
                      ...
                      {categoryIndex < Object.keys(groupedCommands).length - 1 && <Separator className="my-2" />}
                    </div>
                  )
                })}
              </div>
