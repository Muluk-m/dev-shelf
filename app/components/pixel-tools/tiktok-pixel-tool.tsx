...
interface TikTokPixelToolProps {
	embedded?: boolean;
}

export default function TikTokPixelTool({
	embedded = false,
}: TikTokPixelToolProps = {}) {
	...
	const content = (
		<Card>
			<CardHeader>
				...
				<CardDescription>填写 TikTok Pixel 相关参数以触发事件</CardDescription>
			</CardHeader>
								...
								{formData.contents.map((item, index) => (
									<div key={index} className="grid grid-cols-12 gap-2">
										...
										<Input
											...
											onChange={(e) =>
												updateContentItem(index, "content_name", e.target.value)
											}
										/>
										<Input
