...
- **Modular Structure**:
  - `workers/lib/database.ts` - D1 database operations and data mapping
...
**Critical**: The database uses snake_case fields (`is_internal`, `is_external`, `last_updated`) while the frontend uses camelCase (`isInternal`, `isExternal`, `lastUpdated`). Field mapping occurs in `workers/lib/database.ts`.

### API Configuration
The API base URL is dynamically determined in `app/lib/api.ts`:
1. Environment variable `API_BASE_URL` (priority)
2. Browser `window.location.origin` (client-side)
3. Default `http://localhost:5173` (fallback)
