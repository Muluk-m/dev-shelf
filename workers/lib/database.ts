...
export async function getToolCategories(db: D1Database): Promise<ToolCategory[]> {
  const query = `
...
export async function getToolById(db: D1Database, id: string): Promise<Tool | null> {
  const toolQuery = `
    SELECT 
      t.id, t.name, t.description, t.category, t.icon, 
      t.is_internal, t.status, t.last_updated
...
export async function createTool(db: D1Database, tool: Omit<Tool, 'id'>): Promise<string> {
...
export async function updateTool(db: D1Database, id: string, tool: Omit<Tool, 'id'>): Promise<void> {
  try {
    // 开始事务
    await db.batch([
      // 更新工具基本信息
