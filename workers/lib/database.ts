	...
	try {
      ...
      `,
        ...
        `,
					...
					.bind(toolId, env.name, env.label, env.url, env.is_external ? 1 : 0),
			),
			// 插入标签信息
			...tool.tags.map((tag) =>
				db
...
export async function updateTool(
	...
	try {
      ...
      `,
			...
			// 删除旧的环境信息
			db
				.prepare(`DELETE FROM tool_environments WHERE tool_id = ?`)
				.bind(id),
			// 删除旧的标签信息
			db
				.prepare(`DELETE FROM tool_tags WHERE tool_id = ?`)
				.bind(id),
			// 插入新的环境信息
			...tool.environments.map((env) =>
				db
        ...
        `,
					...
					.bind(id, env.name, env.label, env.url, env.is_external ? 1 : 0),
			),
			// 插入新的标签信息
			...tool.tags.map((tag) =>
				db
